...
//! # Usage
//!
//! The global runtime is automatically initialized by the `#[ambitious::main]` macro.
//! You can then use convenience functions like `ambitious::spawn` anywhere in your code.
//!
//! ```ignore
...
//!     let pid = ambitious::spawn(|| async move {
...
///
/// This is called automatically by the `#[ambitious::main]` macro.
...
/// Use `#[ambitious::main]` or call `ambitious::init()` first.
pub fn handle() -> RuntimeHandle {
    RUNTIME
        .get()
        .expect("Ambitious runtime not initialized. Use #[ambitious::main] or call ambitious::init() first.")
        .handle()
}
...
/// ```ignore
/// let pid = ambitious::spawn(|| async move {
///     println!("Hello from {:?}", ambitious::current_pid());
/// });
/// ```
///
/// # Panics
...
mod tests {
    ...
    async fn test_global_register() {
        ...
        let pid = spawn(|| async move {
            ...
            let _ = crate::runtime::recv_timeout(Duration::from_millis(200)).await;
        });
    ...
    async fn test_current_pid() {
        ...
        let spawned_pid = spawn(move || async move {
            ...
            let current = crate::runtime::current_pid();
